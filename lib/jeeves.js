...
(function() {
  var Jeeves, LONG_INTERVAL, LONG_TIMEOUT, SHORT_INTERVAL, SHORT_TIMEOUT, _, _elFuncSuffix, _elementFuncTypes, _shimLevels, asserters, async, logger, ref, webdriver,
  ...
  ref = require('./logger'), logger = ref.logger, _shimLevels = ref._shimLevels;
  ...
  module.exports = Jeeves = (function() {
    ...
    Jeeves.prototype._utils = {
      enableWdLogs: function(driver) {
        driver.on('status', function(info) {
          return logger.test("wd_status << " + info);
        });
        return driver.on('command', function(method, callPath, data) {
          ...
          if (/takeScreenshot/.test(callPath)) {
            data = ((ref1 = JSON.stringify(data)) != null ? ref1.substr(0, 50) : void 0) + '...';
          }
    ...
    function Jeeves() {
      var driver, j, options, ref1, ref2, ref3, ref4, ref5, ref6, wd_config;
      driver = 2 <= arguments.length ? slice.call(arguments, 0, j = arguments.length - 1) : (j = 0, []), options = arguments[j++];
      if (options == null) {
        options = {};
      }
      this.waitForInnerHtmlByCss = bind(this.waitForInnerHtmlByCss, this);
      this.clickElement = bind(this.clickElement, this);
      wd_config = (ref1 = options.wdConfig) != null ? ref1 : {};
      this._wd_capabilites = (ref2 = options.wdCapabilities) != null ? ref2 : {
        browserName: 'chrome'
      };
      this.driver = (ref3 = driver.shift()) != null ? ref3 : null;
      if (this.driver == null) {
      ...
      this._screenshotDir = (ref4 = options.screenshotDir) != null ? ref4 : (process.cwd()) + "/test-results/screenshots";
      SHORT_TIMEOUT = (ref5 = options.shortTimeout) != null ? ref5 : SHORT_TIMEOUT;
      LONG_TIMEOUT = (ref6 = options.longTimeout) != null ? ref6 : LONG_TIMEOUT;
      SHORT_INTERVAL = SHORT_TIMEOUT / 30;
      LONG_INTERVAL = LONG_TIMEOUT / 30;
      return;
    }
    ...
    Jeeves.prototype.init = function(done) {
      return this.driver.init(this._wd_capabilites, (function(_this) {
        return function(error, driverSessionId) {
          var ref1;
          if (error) {
            logger.error("webdriver init error:", (ref1 = error.stack) != null ? ref1 : error);
            return done(error);
          }
    ...
    Jeeves.prototype.loadPageAndWait = function(url, done) {
      ...
      return this.loadPage(url, (function(_this) {
        return function(error) {
          var ref1;
          if (error) {
            logger.error("Error loading page: " + ((ref1 = error.message) != null ? ref1 : error));
            return done(error);
          }
    ...
    Jeeves.prototype.executeClientScript = function() {
      var done, fn, j, params;
      fn = arguments[0], params = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
    ...
    Jeeves.prototype.jsMouseClick = function() {
      var b, button, cssSelector, done, j, jqueryClick, key;
      cssSelector = arguments[0], button = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
    ...
    Jeeves.prototype.mouseClick = function() {
      var b, button, done, j, key;
      button = 2 <= arguments.length ? slice.call(arguments, 0, j = arguments.length - 1) : (j = 0, []), done = arguments[j++];
      key = button.shift() || 'left';
      b = this.MOUSE_KEYS[key];
    ...
    Jeeves.prototype.hasClass = function(elem, className, done) {
      ...
      return elem.getAttribute('class').nodeify(function(error, value) {
        ...
        if (indexOf.call(classes, className) >= 0) {
    ...
    Jeeves.prototype.waitForSomething = function(checkFn, options, done) {
      var _finished, checkCount, finish, finishTimeout, startTime;
    ...
    Jeeves.prototype.waitForUrlToChange = function() {
      var checkUrl, done, expectedUrl, j, matches, options, toFrom;
      expectedUrl = arguments[0], matches = arguments[1], options = 4 <= arguments.length ? slice.call(arguments, 2, j = arguments.length - 1) : (j = 2, []), done = arguments[j++];
      if (matches == null) {
        matches = true;
      }
      ...
      return this.waitForSomething(checkUrl, {
        ...
        return function(error) {
          var ref1;
          logger.test('-- waitForUrlToChange done. Attempting to get url', [expectedUrl, (ref1 = error != null ? error.message : void 0) != null ? ref1 : 'no error']);
          if (error) {
            return done(error);
          }
    ...
    Jeeves.prototype.waitForInnerHtmlByCss = function() {
      var done, htmlToReturn, j, options, regex, selectorValue;
      selectorValue = arguments[0], regex = arguments[1], options = 4 <= arguments.length ? slice.call(arguments, 2, j = arguments.length - 1) : (j = 2, []), done = arguments[j++];
  ...
  _.each(_elementFuncTypes, function(type) {
    ...
    Jeeves.prototype['doubleClickElement' + _elFuncSuffix(type)] = function() {
      var done, j, offsets, ref1, ref2, selectorValue, xOffset, yOffset;
      selectorValue = arguments[0], offsets = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
      logger.test("@doubleClickElement" + (_elFuncSuffix(type)) + " using selectorValue: " + selectorValue);
      xOffset = (ref1 = offsets.shift()) != null ? ref1 : 0;
      yOffset = (ref2 = offsets.shift()) != null ? ref2 : 0;
    ...
    Jeeves.prototype['getText' + _elFuncSuffix(type)] = function() {
      var callCount, done, j, opts, selectorValue;
      selectorValue = arguments[0], opts = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
    ...
    Jeeves.prototype['waitForElement' + _elFuncSuffix(type)] = function() {
      var done, interval, j, options, selectorValue, timeout;
      selectorValue = arguments[0], options = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
    ...
    Jeeves.prototype['waitForAndGetElement' + _elFuncSuffix(type)] = function() {
      var done, interval, j, options, selectorValue, timeout, visibleElem, waitForMethod;
      selectorValue = arguments[0], options = 3 <= arguments.length ? slice.call(arguments, 1, j = arguments.length - 1) : (j = 1, []), done = arguments[j++];
    ...
    Jeeves.prototype['waitForElementText' + _elFuncSuffix(type)] = function() {
      var done, interval, j, options, selectorValue, text, timeout;
      selectorValue = arguments[0], text = arguments[1], options = 4 <= arguments.length ? slice.call(arguments, 2, j = arguments.length - 1) : (j = 2, []), done = arguments[j++];
